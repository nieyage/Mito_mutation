...
python organize_files_parallel.py \
  -i /md01/nieyg/project/mito_mutation/03_human_PBMC/PBMC_lib5_output/SNV_Calling_allcell_minreads1 \
  -a /data/R01/renwx5/MT_mution/mutation_outs/sample_annotation_new/lib5_annotation.csv \
  -o /md01/nieyg/project/mito_mutation/03_human_PBMC/PBMC_lib5_output/output_minreads1 \
  -w 20  # 使用16个工作线程

bash /md01/nieyg/pipeline/mito_mutation/01_human/process_donors.sh \
  --base-dir /md01/nieyg/project/mito_mutation/03_human_PBMC/PBMC_lib5_output/output_minreads1 \
  --output-dir /md01/nieyg/project/mito_mutation/03_human_PBMC/PBMC_lib5_output/donor_results_minreads1 \
  --threads 32 \
  --workers 20
...
# 步骤1: 读取变异稀疏矩阵数据
variant_file <- "./Donor1_variant_sparse_matrix_withoutstrand.tsv.gz"
...
somatic_snv<- "./Donor1_somatic.tsv"
...
plot_mut<- plot_and_save_mutation_signature(
   mutation_data,
   output_prefix = "lib5_donor_minreads1",
   title_prefix = "lib5_donor_minreads1",width = 12, height = 12
 )
