...
# 设置路径
csv_file="/md01/nieyg/project/mito_mutation/02_mm10_pipeline/03_singlecell_SNV/test.csv"
output_base="/md01/nieyg/project/mito_mutation/02_mm10_pipeline/03_singlecell_SNV/SNV_calling"
...
process_barcode() {
    ...
    awk -v start=8300 -v end=8900 -v new_start=1 'BEGIN{OFS="\t"} $2>=start&&$2<=end{$2=$2-start+new_start;print}' "$shifted_mpileup" > "${output_prefix}_temp1"
    awk -v start=601 -v end=15700 'BEGIN{OFS="\t"} $2>=start&&$2<=end{print}' "$unshifted_mpileup" > "${output_prefix}_temp2"
    awk -v start=7701 -v end=8299 -v new_start=15701 'BEGIN{OFS="\t"} $2>=start&&$2<=end{$2=$2-start+new_start;print}' "$shifted_mpileup" > "${output_prefix}_temp3"
