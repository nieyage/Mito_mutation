...
celltype_file <- "/md01/nieyg/project/lineage_tracing/heart_regeneration/00_data/AR3_data/scATAC/somatic_mutation/AR3_C4_celltype_info.txt"

if (file.exists(celltype_file)) {
  celltype_df <- read.table(celltype_file)
  colnames(celltype_df)<- c("barcode","celltype")
  # 检查必要的列是否存在
  if ("barcode" %in% colnames(celltype_df) && "celltype" %in% colnames(celltype_df)) {
...
p <- ggplot() +
  geom_line(data = plot_data, 
            ...
            size = 0.8) +
  scale_color_manual(values = my47colors)+
...
barcode_file<- "../barcodes_BMMC27m.txt"
...
stats<- read.csv("barcode_mtDNA_stats_from_bam.csv")
...
plot_mut<- plot_and_save_mutation_signature(
   mutation_data,
   output_prefix = "AR3",
   title_prefix = "AR3",width = 8, height = 8
 )
